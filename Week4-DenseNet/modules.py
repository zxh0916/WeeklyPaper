...
class BRC(nn.Module):
    ...
    def forward(self, x):
        if self.batch_norm:
            x = self.bn(x)
        return self.dropout(self.conv(self.relu(x)))
...
class Bottleneck(nn.Module):
    def __init__(self,
                 ...
                 dropout=0):
        super().__init__()
        self.BRC1 = BRC(in_channels, 4*k, kernel_size=1, stride=1, padding=0, batch_norm=batch_norm, dropout=dropout)
        self.BRC2 = BRC(4*k, k, batch_norm=batch_norm, dropout=dropout)
...
class DenseBlock(nn.Module):
    def __init__(self, k_0, k, block, num_layer, batch_norm=True, dropout=0):
        ...
        for i in range(self.num_layer):
            self.layers.append(block(k_0 + k*i, k, batch_norm=batch_norm, dropout=dropout))
    ...
    #     outputs = [x]
    #     for i in range(self.num_layer):
    #         outputs.append(self.layers[i](torch.concat(outputs[:i+1], dim=1)))
...
class DenseNet_ImageNet(nn.Module):
    def __init__(self, k, theta, block, num_layers, num_classes, batch_norm=True, dropout=0):
        super().__init__()
        self.batch_norm = batch_norm
        ...
        for i, num_layer in enumerate(num_layers):
            self.blocks[i].append(DenseBlock(self.in_channels, k, block, num_layer, self.batch_norm, dropout))
...
def DenseNet121_BC(k, theta, num_classes, batch_norm=True, device=device):
    return DenseNet_ImageNet(k,
                             theta,
                             Bottleneck,
                             [6, 12, 24, 16],
                             num_classes,
...
def DenseNet169_BC(k, theta, num_classes, batch_norm=True, device=device):
    return DenseNet_ImageNet(k,
                             theta,
                             Bottleneck,
                             [6, 12, 32, 32],
...
def DenseNet201_BC(k, theta, num_classes, batch_norm=True, device=device):
    return DenseNet_ImageNet(k,
                             theta,
                             Bottleneck,
                             [6, 12, 48, 32],
...
def DenseNet264_BC(k, theta, num_classes, batch_norm=True, device=device):
    return DenseNet_ImageNet(k,
                             theta,
                             Bottleneck,
                             [6, 12, 64, 48],
